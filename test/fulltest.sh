...
test_nccl() {
    ...
    local nccl_diag
    nccl_diag=$(mktemp "${TMPDIR:-/tmp}/fulltest-nccl.XXXXXX")
    cleanup_nccl_diag() {
        rm -f "$nccl_diag"
    }
...
# particularly under workloads with high transient current (integer/bitwise
# compute such as ECDSA/hashing).
#
...
run_burn_monitor() {
    ...
    # at the end of the monitor. BURN_TELEMETRY_FILE is exported so other
    # functions can find it if needed.
...
#   1. power draw consistently >= POWER_ANOMALY_DELTA_W below the per-sample
#      median across all GPUs,
#   2. fan speed at or above POWER_ANOMALY_FAN_PCT in the same samples,
#   3. for at least POWER_ANOMALY_FRAC_PCT% of post-warmup samples.
#
# The driver does NOT report this as throttling, so it is invisible to
# clocks_throttle_reasons.active. This check is an early-warning signal for
# the 12V-2x6 / 12VHPWR connector failure mode that can precede Xid 79.
#
# Sets BURN_POWER_ANOMALY_RC=1 if any GPU is flagged.
# ─────────────────────────────────────────────────────────────────────────────
