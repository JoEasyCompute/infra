...
SCRIPT_CUDA_TEST="${PROVISION_DIR}/code.sh"
SOURCE_CUDA_TEST="${PROVISION_DIR}/code.cu"
...
missing=0
for script in "$SCRIPT_BASE_INSTALL" "$SCRIPT_DOCKER_INSTALL" "$SCRIPT_FULLTEST" "$SCRIPT_CUDA_TEST"; do
    if [[ ! -f "$script" ]]; then
...
if [[ ! -f "$SOURCE_CUDA_TEST" || ! -r "$SOURCE_CUDA_TEST" ]]; then
    error "CUDA source not found or unreadable: ${SOURCE_CUDA_TEST}"
    missing=$((missing + 1))
fi
(( missing > 0 )) && {
