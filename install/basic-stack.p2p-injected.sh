...
P2P_OPEN=0
RUNFILE="/root/NVIDIA-Linux-x86_64-570.148.08.run"
...
# If P2P_OPEN is enabled, force 570 headless userspace (tinygrad open modules require 570.148.08)
if [[ $P2P_OPEN -eq 1 ]]; then
  if [[ "$DRIVER_VER" != "570" ]]; then
    echo "[WARN] --p2p-open forces --driver 570 (was $DRIVER_VER)"; DRIVER_VER="570";
  fi
  if [[ "$DRIVER_MODE" != "headless" ]]; then
    echo "[WARN] --p2p-open forces --mode headless (was $DRIVER_MODE)"; DRIVER_MODE="headless";
  fi
fi
...
# -------------------------- Tinygrad P2P Open Modules (helper) -----------------
secure_boot_check() {
  if command -v mokutil >/dev/null 2>&1; then
    if mokutil --sb-state 2>/dev/null | grep -qi "SecureBoot enabled"; then
      echo "ERROR: Secure Boot is enabled. Disable it or sign the modules before proceeding." >&2
      exit 1
    fi
  fi
}
...
install_tinygrad_p2p_open() {
  ...
  apt_retry apt-get install $APT_FLAGS build-essential dkms linux-headers-$(uname -r) git wget curl pkg-config
  ...
  if [[ ! -f "$runfile" ]]; then
    ...
    wget -q --show-progress -O "$runfile" "$url"
  fi
  chmod +x "$runfile"

  # Install userspace only (no kernel modules, no X/GL stacks)
  ...
  # Fetch tinygrad fork at exact tag and build
  ...
  # Load headless modules
  modprobe nvidia
  modprobe nvidia_uvm

  # Persistence for compute
...
if [[ $P2P_OPEN -eq 1 ]]; then
  log "Installing NVIDIA 570.148.08 userspace + tinygrad P2P OPEN kernel modules (headless)"
  install_tinygrad_p2p_open
else
  if [[ "$DRIVER_MODE" == "headless" ]]; then
    PKGS=("nvidia-headless-${DRIVER_VER}-open" "nvidia-utils-${DRIVER_VER}")
