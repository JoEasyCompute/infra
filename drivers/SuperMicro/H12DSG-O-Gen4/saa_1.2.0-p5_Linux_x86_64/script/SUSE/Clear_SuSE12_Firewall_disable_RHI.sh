    ...
    then
        ...
        then
            ...
            if [ $? -ne 0 ]; then
                ...
                ip addr delete 169.254.3.1/255.255.255.0 dev $INTERFACE
                ip link set dev $INTERFACE down
                echo "Disable Redfish Host Interface."
            fi

            # Remove RHI from firewall whitelist
            ...
            if [ ! -z "$tmp" ]
            then
                tmp2=$(echo ${tmp} | sed "s/\"${INTERFACE}\s\+/\"/g;s/\s\+${INTERFACE}\s\+/ /g;s/\s\+${INTERFACE}\"/\"/g")
                sed -i "s/${tmp}/${tmp2}/" /etc/sysconfig/SuSEfirewall2
                systemctl restart SuSEfirewall2
                echo "Remove Redfish Host Interface from firewall whitelist."
            fi
            fi
            exit 0
        fi
