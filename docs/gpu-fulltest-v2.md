...
```bash
./test/gpu-fulltest-v2.sh [test...] [-test...] [--gpu <index[,index...]>] [--burn-duration <seconds>] [--node-stress-minutes <m>] [--clean] [--list] [--help]
```
...
./test/gpu-fulltest-v2.sh -code -memtest
...
- You can exclude any named test by prefixing it with `-` or by passing `--exclude <test>`. Repeat the flag or prefix for multiple exclusions, for example `-code -memtest` or `--exclude code --exclude memtest`. Exclusions are applied after normal test selection.
...
- In plain terms: thermal or fan-limit findings are recorded as remarks, and the stress lanes only fail on an actual GPU crash or other hard failure signal.
