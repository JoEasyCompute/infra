...
- CUDA 13 now maps to the `cu130` wheel family; `torch` / `torchvision` / `torchaudio` installs are force-refreshed so stale venv contents do not keep an old wheel family alive between runs.
