...
Installs: `apt-transport-https`, `ca-certificates`, `curl`, `gnupg`, `wget`, `linux-headers-$(uname -r)`, **`linux-modules-extra-$(uname -r)`** (required for `amdgpu-dkms`, not needed for NVIDIA), `linux-headers-generic`, `git`, `cmake`, `build-essential`, `dkms`, `gcc-11`, `gcc-12`, `g++-11`, `g++-12`, `python3`, `python3-pip`, `python3-venv`, `python3-setuptools`, `python3-wheel`, `chrony`, `smartmontools`, `stress-ng`, `fio`, `lm-sensors`, `lvm2`, `mdadm`, `lsof`, `ioping`, `pciutils`, `nvme-cli`, `ipmitool`, `jq`, `dmidecode`, `lshw`, `bpytop`, `mokutil`, plus `uv` via Astral's standalone installer to `/usr/local/bin`.
