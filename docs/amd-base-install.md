...
Installs: `apt-transport-https`, `ca-certificates`, `curl`, `gnupg`, `wget`, `debconf-utils`, `linux-headers-$(uname -r)`, **`linux-modules-extra-$(uname -r)`** (required for `amdgpu-dkms`, not needed for NVIDIA), `linux-headers-generic`, `git`, `cmake`, `build-essential`, `dkms`, `gcc-11`, `gcc-12`, `g++-11`, `g++-12`, `python3`, `python3-pip`, `python3-venv`, `python3-setuptools`, `python3-wheel`, `chrony`, `smartmontools`, `stress-ng`, `fio`, `lm-sensors`, `ethtool`, `iperf3`, `rsync`, `xorriso`, `squashfs-tools`, `grub-common`, `grub-pc-bin`, `grub-efi-amd64-bin`, `lvm2`, `mdadm`, `lsof`, `ioping`, `pciutils`, `usbutils`, `nvme-cli`, `ipmitool`, `jq`, `fzf`, `ripgrep`, `fd-find`, `bat`, `dmidecode`, `lshw`, `bpytop`, `mokutil`, plus `yq` via the official GitHub release binary and `uv` via Astral's standalone installer to `/usr/local/bin`. The installer also preseeds `iperf3` to start its daemon automatically so apt does not prompt during install, and it creates `fd` and `bat` compatibility symlinks when the Ubuntu package names expose `fdfind` and `batcat`.
